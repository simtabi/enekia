...
trait Subdomain
{
    ...
    /**
     * List of reserved/excluded subdomains
     *
     * @var array
     */
     ...
     * @var bool
     */
    protected bool $checkSubdomain = false;

    /**
     * @param string $separatorSign
     * @return static
     */
    public function checkSubdomain(string $separatorSign = ':'): static
    {
    ...
    public function validateSubdomain($attribute, $value): bool
    {
        $reserved = [
        ...
        $this->excludedWords = !empty($this->excludedWords) ? array_merge($this->excludedWords, $reserved) : $reserved;

        if (!preg_match('/^[A-Za-z0-9](?:[A-Za-z0-9\-]{0,61}[A-Za-z0-9])?$/', $value)) {
            return false;
        }

        if (in_array($value, $this->excludedWords)) {
            return false;
        }
