...
use Simtabi\Enekia\Laravel\Helpers\ISO;
use Exception;
...
class Language extends AbstractRule implements Rule
{
    ...
    public function passes($attribute, $value) : bool
    {
        ...
        if ($this->name) {
           ...
           return !empty(ISO::ISO639()->name($value));
        }elseif ($this->alpha2){
            $this->messageKey = 'alpha2';
            return !empty(ISO::ISO639()->alpha2($value));
        }
        // ISO specific methods
        elseif ($this->iso639_1){
            $this->messageKey = 'iso639_1';
            return !empty(ISO::ISO639()->iso639_1($value));
        }elseif ($this->iso639_2B){
            $this->messageKey = 'iso639_2B';
            return !empty(ISO::ISO639()->iso639_2B($value));
        }elseif ($this->iso639_2T){
            $this->messageKey = 'iso639_2T';
            return !empty(ISO::ISO639()->iso639_2T($value));
        }
